...
<section>
	<?php require "top_header_link.php"; ?>	
</section>
...
<section>
	<?php //require "Set_file.php"; ?>	
</section>
	
	
<section>
    <?php require "left_1.php"; ?>
	<?php require "Left_2.php"; ?>
</section>
...
<div class="col-sm-12 main-top-body"><!----------------main-------body-----of----dashborad-------->
   					<div class="row">
   						<div class="col-sm-6"><h5>Sales </h5></div>
   						<div class="col-sm-6">
...
<div class="row pt-3"><!-----------row--------n012--------------->
   <div class="col-sm-12">
     <div class="card p-3" id="personal-info">
         <h5 ><span><img src="icons/icons8-buy-48 (1).png" class="img-fluid"></span> <span class="toplink">Sales Information:</span></h5>
...
<form name="FrmFilter" class="Frmstar786" method="post" action="store-issue.php">  
   ...
   <div class="row">
	...
	<div class="form-group">
		<!----<label>Date</label>----->
		<input type="date" name="PDATE" value="<?php echo $PDATE; ?>" class="form-control no-rad no-padd" placeholder="Date ..." required />
	</div>
	</div>
   ...
   <div class="col-md-3">
	<div class="form-group">
		<!----<label>Vendor</label>----->
		<select name="VID" class="form-control no-rad no-padd" required>
...
?>
   ...
   <div class="col-md-6">
	<div class="form-group">
		<!---<label>Note</label>------>
		<input type="text" name="DETAIL" id="DETAIL" class="form-control no-rad no-padd2" value="<?php echo $DETAIL ;?>" />
	</div>
	</div>
    </div>
    	...
    	<table width="100%" border="1" class="table table-bordered order-list TblReport" id="TblReport">
		    ...
		    <tbody>
		        ...
		        <tbody>
				    ...
				    <td width="20%">
						<select name="catid[]" id="catid" class="form-control selectpicker no-rad no-padd2" data-live-search="true" onChange="getStoreAssets(this.value,x=-1)">
				    ...
				    <td width="30%">
				    	...
				    	<div class="DivProd">
				    		<select name="PRODID[]" id="getppr" class="form-control getProd no-rad no-padd2" data-live-search="true"  onChange="showStocks(this.value,x=-1)"></select>
				     ...
				     <td width="20%" align="center"><div id="DivStock"><input type="text" name="PRICE[]" id="PRICE" class="form-control PRICE no-rad no-padd2" step="any" value=""></div></td>



				     <!-- onchange="getTotal(this.value,x=-1)" -->
				     <td width="20%"><input step="any" type="number" name="QTY[]" class="form-control no-rad no-padd2" id="QTY" class="form-control " value="">
					...
					<td style="width: 20%;">
				    	...
				    	<input type="text" name="Tax[]" class="form-control no-padd2 no-rad " id="tax" value="">

				      </td>
				        <td style="width: 30%;">
				    		<input type="text" name="NOTE[]" class="form-control NOTE no-rad no-padd2" value="">
				    	</td>
...
<script type="text/javascript">
 ...
 $("#addrow").on("click", function () {
	        ...
	        cols += '<td><select name="catid['+counter+']" id="catid['+counter+']" class="form-control selectpicker no-rad no-padd2" data-live-search="true" onChange="getStoreAssets(this.value,x='+counter+')"><option value="">Select Grupo</option><?php	$resultCat = mysqli_query($link,"SELECT * FROM prodcat"); if (mysqli_num_rows($resultCat) == 0) {echo "<option>No Names Present</option>";} else{ while($row = mysqli_fetch_array($resultCat)){ $CATEGORYID = $row['CATID']; $CATEGORYNAME = $row['CATNAME'];?><option value="<?php echo($CATEGORYID); ?>"><?php echo($CATEGORYNAME); ?> </option> <?php } } ?></select></td>';
	        cols += '<td><div class="getProductX" style="display:block !important;"><select name="PRODID[]" id="PRODID['+counter+']" class="form-control getProdX no-rad no-padd2" data-live-search="true" onChange="showStocks(this.value,x='+counter+')"></select></div><select name="ASTID[]" id="ASTID['+counter+']" style="display: none;"></select></td>';
	        cols += '<td align="center"><div id="DivStock"><input type="text" name="PRICE[]" id="PRICEX['+counter+']" step="any" class="form-control no-rad no-padd2" value=""></div></td>';
	        cols += '<td><input type="number" step="any" name="QTY[]" id="QTYX['+counter+']" class="form-control no-rad no-padd2" value=""></td>';
	        ...
	        cols += '<td><input type="text" name="Tax[]" class="form-control no-padd2 no-rad " id="tax['+counter+']" value=""></td>';
	        cols += '<td><input type="text" name="NOTE[]" id="NOTE['+counter+']" class="form-control no-rad no-padd2" value=""></td>';
...
</div><!----------------main-------body-----of----dashborad-------->
</div><!----------------main----row---body-----of----dashborad-------->
